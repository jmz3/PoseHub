...
It subscribes to all the topics that match the specified pattern, which is
"object" by default,
...
#include <string>
#include <vector>
// Function to get the list of topics that match the input pattern using regular
// expressions
std::vector<std::string> getTopicsWithPattern(const std::string &pattern) {
  ...
  // Exclude the topics that contains "noise" or "noisy" to avoid duplicate
  // noise addition
  std::regex exclude_pattern(".*noise.*|.*noisy.*");
  ...
  for (const auto &topic_info : all_topics) {
    try {
      if (std::regex_match(topic_info.name, regex_pattern) &&
          !std::regex_match(topic_info.name, exclude_pattern)) {
...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("sensor_name", sensor_name, "HoloLens");
  ROS_INFO("The sensor name is: %s", sensor_name.c_str());

  SensorNoisyReading noisy_camera_(sensor_name, filtered_topics, nh);
  noisy_camera_.setSensorNoise(
      new double[36]{0.01, 0, 0,     0, 0,    0, 0, 0.012, 0, 0,    0, 0,
                     0,    0, 0.034, 0, 0,    0, 0, 0,     0, 0.01, 0, 0,
